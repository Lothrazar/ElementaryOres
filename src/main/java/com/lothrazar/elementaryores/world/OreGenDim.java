...
public class OreGenDim extends OreFeature {

  public OreGenDim(Function<Dynamic<?>, ? extends OreFeatureConfig> p_i51472_1_) {
    super(p_i51472_1_);
  }
                ...
                for(int j2 = l; j2 <= k1; ++j2) {
                  ...
                  if (d8 * d8 + d9 * d9 + d10 * d10 < 1.0D) {
                    ...
                    if (!bitset.get(k2)) {
                      ...
                      if (worldIn.getBlockState(blockpos$mutableblockpos).getBlock() == Blocks.END_STONE) {//  if (config.target.func_214738_b().test(worldIn.getBlockState(blockpos$mutableblockpos))) {
